...
if (!uri) {
    console.error("monogdb_uri not found in .env");
    process.exit(1);
}
...
async function connectToMongo() {
    try {
        ...
        db = client.db('certificate_generator');
        console.log("Successfully connected to MongoDB!");

        const participants = db.collection('participants');
        await participants.createIndex({ serial_number: 1 });
        const downloads = db.collection('certificate_downloads');
        await downloads.createIndex({ serial_number: 1 });
        const organizations = db.collection('organizations');
        await organizations.createIndex({ category: 1 });
        ...
        if (!adminCheck) {
            await adminAuth.insertOne({ role: 'superadmin', password_hash: initPass, updated_at: new Date() });
        }
    } catch (err) {
...
app.post('/api/sql', async (req, res) => {
    try {
        ...
        if (sql.includes("INSERT OR REPLACE INTO PARTICIPANTS")) {
            // args: [participant_id, name, serial_number, institute_name, atari_zone, training_dates, created_at, is_restricted]
        ...
        if (sql.startsWith("UPDATE PARTICIPANTS SET NAME = ?")) {
            // args: [name, serial_number, institute_name, atari_zone, training_dates, is_restricted, id, serial_number]
            await db.collection('participants').updateOne(
        ...
        if (sql.includes("SELECT * FROM CERTIFICATE_DOWNLOADS ORDER BY DOWNLOAD_TIME DESC")) {
            ...
            return res.json({ rows: mappedRows });
        }
        ...
        if (sql.includes("SELECT * FROM ADMIN_AUTH WHERE ROLE = 'SUPERADMIN'")) {
            ...
            return res.json({ rows: auth ? [auth] : [] });
        }
        ...
        if (sql.includes("SELECT * FROM SYSTEM_CONFIG WHERE KEY = 'CERTIFICATE_SETTINGS'")) {
            ...
            return res.json({ rows: config ? [config] : [] });
        }
...
// Serve frontend files in production logic
...
const PORT = 5000;
app.listen(PORT, () => {
    console.log(`MongoDB Backend proxy server running on port ${PORT}`);
});
